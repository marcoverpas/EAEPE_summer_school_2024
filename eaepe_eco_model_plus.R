# IO-SFC MODEL (BASED ON MODEL PC) + ECOSYSTEM + EXPERIMENT
...
beta_e = 0.07 #CO2 intensity of non-renewable energy sources
...
plot(h_s[2,2:nPeriods]-h_h[2,2:nPeriods], type="l", col="green",lwd=3,lty=1,font.main=1,cex.main=1.5,
...
#Figure 2: disposable income and consumption after increase in temperature elasticity of consumption
plot(yd[2,45:nPeriods]/yd[1,45:nPeriods],type="l", col=1, lwd=2, lty=1, font.main=1,cex.main=1.5,
     main="Figure 2  Disposable income and consumption \n after increase in alpha12",
     ylab = 'Index',xlab = 'Time',ylim = range(0.8,1.05),
     cex.axis=1.5,cex.lab=1.5)
lines((cons[2,45:nPeriods]*p_c[2,45:nPeriods])/(cons[1,45:nPeriods]*p_c[1,45:nPeriods]),type="l",lwd=2,lty=2,col="purple1")
legend("bottomright",c("Disposable income","Nom. consumption"),  bty = "n",
       cex=1.5, lty=c(1,2), lwd=c(2,2), col = c(1,"purple1"), box.lty=0)
abline(h=1,col=1,lty=3)

#Figure 3: reserves depletion rates after increase in temperature elasticity of consumption 
plot(100*(mat[2,45:nPeriods]/k_m[2,44:(nPeriods-1)])-100*(mat[1,45:nPeriods]/k_m[1,44:(nPeriods-1)]),type="l",
     col="coral3", lwd=3, lty=1, font.main=1,cex.main=1.5,
     main="Figure 3  Reserves depletion rates \n after increase in alpha12",
     ylab = 'Index (% change)',xlab = 'Time',cex.axis=1.5,cex.lab=1.5,ylim=range(-0.15,0.05))
lines(100*(en[2,45:nPeriods]/k_e[2,44:(nPeriods-1)])-100*(en[1,45:nPeriods]/k_e[1,44:(nPeriods-1)]),col="cyan4", lwd=3, lty=1)
legend("bottomright",c("Matter","Energy"),  bty = "n",
       cex=1.5, lty=c(1,1), lwd=c(3,3), col = c("coral3","cyan4"), box.lty=0)
abline(h=0,col=1,lty=3)

#Figure 4: atmospheric temperature after increase in temperature elasticity of consumption 
plot(temp[2,45:nPeriods]-temp[1,45:nPeriods],type="l",
